...
class CommandCenterView(QWidget):
    ...
    def add_hop(self, hop: TracerouteHop):
        """Merge hop by hop_num. After finish (_probing is None), never resurrect probing."""
        # Prefer keeping a better hostname if update is empty
        prev = next((h for h in self._hops if h.hop_num == hop.hop_num), None)
        if prev and hop.hostname is None and prev.hostname:
            hop.hostname = prev.hostname
        if prev and hop.role in (None, "") and prev.role:
            hop.role = prev.role
        self._hops = [h for h in self._hops if h.hop_num != hop.hop_num]
        self._hops.append(hop)
        self._hops.sort(key=lambda h: h.hop_num)
        # Only advance live probing skeleton while a run is in progress
        if self._probing is not None and hop.state != "probing":
            self._probing = hop.hop_num + 1
        self._refresh()
        if self._probing is None:
            # post-finish PTR update: refresh selection detail if needed
            if self._selected == hop.hop_num:
    ...
    def finish_run(
        ...
        # Clear probing first so visible count is stable
        self._probing = None
        cleaned = []
        for h in hops:
            if h.state == "probing":
                h.state = "timeout" if h.timed_out or not h.ip else "ok"
            cleaned.append(h)
        self._hops = cleaned
        self._refresh()
        visible = self.all_hops_with_pc()
        n = len(visible)  # includes This PC — matches table/card rows
        timeouts = [h for h in cleaned if h.timed_out]
        dest = ""
        if cleaned:
            last = cleaned[-1]
            dest = last.hostname or last.ip or "destination"
        if cancelled:
            self.set_status(f"Cancelled - {n} hops shown.")
        elif error:
            ...
            if timeouts:
                th = timeouts[0]
                extra = f"{len(timeouts)} timeout at hop {th.hop_num}"
                self.set_status(
                    f"Path complete - {n} hops to {dest} (incl. This PC). {extra}."
                )
            else:
                self.set_status(
                    f"Path complete - {n} hops to {dest} (incl. This PC)."
                )
            w = worst_hop(visible)
            if w:
    ...
    def all_hops_with_pc(self) -> List[TracerouteHop]:
        """Visible hops for table/cards/overview.

        Count rule (everywhere): len(all_hops_with_pc()) after finish =
        This PC + path hops. Never include a probing placeholder once finished.
        """
        out = []
        if self._pc:
            out.append(self._pc)
        for h in self._hops:
            if self._probing is None and h.state == "probing":
                continue  # drop stale probing rows after finish/cancel
            out.append(h)
